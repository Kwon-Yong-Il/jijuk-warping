...
class PixelExtractor:
    def __init__(self, root):
        ...
        self.magnifier_canvas = tk.Canvas(self.image_frame, width=self.magnifier_size, height=self.magnifier_size, bg="white")
        # self.magnifier_canvas.pack(side="top", fill="both", expand=False)  # 주석 처리
    ...
    def update_magnifier(self, event):
        if self.tk_image:
            ...
            crop_left = int(max(x - self.magnifier_size / (2 * magnification_factor), 0))
            crop_top = int(max(y - self.magnifier_size / (2 * magnification_factor), 0))
            crop_right = int(min(crop_left + self.magnifier_size / magnification_factor, self.original_image.width))
            crop_bottom = int(min(crop_top + self.magnifier_size / magnification_factor, self.original_image.height))

            # Ensure that the coordinates are valid
            crop_left = min(crop_left, crop_right - 1)
            crop_top = min(crop_top, crop_bottom - 1)

            # Check if the coordinates are valid before calling the crop method
            if crop_left < crop_right and crop_top < crop_bottom:
                magnified_region = self.original_image.crop((crop_left, crop_top, crop_right, crop_bottom))
                magnified_region = magnified_region.resize((self.magnifier_size, self.magnifier_size), resample=Image.BICUBIC)

                # Create an image with a white border using ImageDraw
                magnified_with_border = Image.new("RGB", (self.magnifier_size + 4, self.magnifier_size + 4), "white")
                draw = ImageDraw.Draw(magnified_with_border)
                draw.rectangle([0, 0, self.magnifier_size + 3, self.magnifier_size + 3], outline="white")

                self.magnifier_image = ImageTk.PhotoImage(magnified_region)

                # Update the magnifier position based on the mouse pointer coordinates
                magnifier_x, magnifier_y = x - self.magnifier_size / 2, y - self.magnifier_size / 2

                # Ensure the magnifier is within the bounds of the original image
                magnifier_x = max(0, min(magnifier_x, self.original_image.width - self.magnifier_size))
                magnifier_y = max(0, min(magnifier_y, self.original_image.height - self.magnifier_size))

                # Delete existing magnifier image if it exists
                self.canvas.delete("magnifier")

                # Create an image on the canvas for the magnified image
                self.canvas.create_image(magnifier_x, magnifier_y, anchor=tk.NW, image=self.magnifier_image, tags="magnifier")

                # Draw crosshair in the center of the magnified image
                center_x = magnifier_x + self.magnifier_size / 2
                center_y = magnifier_y + self.magnifier_size / 2

                # Adjust the crosshair length
                crosshair_length = 100  # 조절하려는 크기로 변경
                
                # Draw thick horizontal line
                self.canvas.create_line(center_x - crosshair_length, center_y, center_x + crosshair_length, center_y, fill="blue", tags="magnifier", width=3)

                # Draw thick vertical line
                self.canvas.create_line(center_x, center_y - crosshair_length, center_x, center_y + crosshair_length, fill="blue", tags="magnifier", width=3)
            ...
            with open(dogak_list_file_path, "r") as file:
                ...
                for line in lines:
                    ...
                    if len(fields) >= 3:
                        ...
                        # Debugging print
                        print(f"Comparing: {image_filename[:15]} == {os.path.basename(target_filename)[:15]}")

                        # Check if the first 15 characters of the image filename match the target filename
                        if image_filename[:15] == os.path.basename(target_filename)[:15]:
                            user_x, user_y = x, y
    ...
    def on_click(self, event):
        ...
        # Bind the magnifier update function to mouse motion
        self.canvas.bind("<Motion>", self.update_magnifier)
    ...
    def on_canvas_configure(self, event):
        if self.tk_image:
            ...
            # Update the magnifier position when the canvas is scrolled
            magnifier_x, magnifier_y = self.get_magnifier_position()
            self.canvas.coords("magnifier", magnifier_x, magnifier_y)

    def get_magnifier_position(self):
        # Get the current mouse pointer coordinates within the canvas
        x, y = self.canvas.canvasx(self.root.winfo_pointerx()), self.canvas.canvasy(self.root.winfo_pointery())

        # Calculate the position to place the magnified image above the original image
        magnifier_x = x - self.magnifier_size / 2
        magnifier_y = y - self.magnifier_size / 2

        # Ensure the magnifier is within the bounds of the original image
        magnifier_x = max(0, min(magnifier_x, self.original_image.width - self.magnifier_size))
        magnifier_y = max(0, min(magnifier_y, self.original_image.height - self.magnifier_size))

        return magnifier_x, magnifier_y
