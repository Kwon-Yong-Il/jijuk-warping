            ...
            with open(dogak_list_file_path, "r") as file:
                ...
                for line in lines:
                    ...
                    if len(fields) >= 3:
                        ...
                        # Debugging print
                        print(f"Comparing: {os.path.splitext(image_filename)[:15]} == {os.path.splitext(os.path.basename(target_filename))[:15]}")

                        # Check if the image filename matches the target filename (considering only the first 15 characters)
