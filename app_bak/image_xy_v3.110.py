...
class PixelExtractor:
    ...
    def load_target_dogak_coordinates(self, target_filename):
        ...
        try:
            coordinates_dict = {}  # Dictionary to store image coordinates
            ...
            if os.path.exists(dogak_file_path):
                with open(dogak_file_path, "r") as file:
                    ...
                    for line in lines:
                        ...
                        # Parse the line and store coordinates in the dictionary
                        fields = line.strip().split(',')
                        if len(fields) >= 3:
                            image_filename, x, y = fields[0], float(fields[1]), float(fields[2])
                            coordinates_dict[image_filename] = (x, y)

            # Check if target_filename is in the dictionary
            if os.path.splitext(target_filename)[0] in coordinates_dict:
                user_x, user_y = coordinates_dict[os.path.splitext(target_filename)[0]]
