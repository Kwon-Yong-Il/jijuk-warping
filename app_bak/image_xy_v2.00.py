...
class PixelExtractor:
    ...
    def save_to_excel(self, image_filename, positions):
        ...
        for i, pos in enumerate(positions, start=1):
            user_x, user_y, x, y = pos
            self.sheet.append([image_filename, f"Point {i}", f"{user_x}, {user_y}", f"{x}, {y}", clicked_time])
    ...
    def on_click(self, event):
        ...
        if self.click_count == 0:
            ...
            if len(self.click_coords) > 0:
                ...
                x2, y2 = x + alpha, y + beta
                self.click_coords.append((x, y, x2, y2))
                self.click_count += 1
            else:
                messagebox.showinfo("Error", "Please click the first point before the second.")
        elif self.click_count == 2:
            if len(self.click_coords) > 1:
                x2, _, _, y1 = self.click_coords[1]
                x3, y3 = x + alpha, y1 + beta
                self.click_coords.append((x2, y1, x3, y3))

                # Correctly get the image filename for the current image
                ...
                # Reset click count and clear canvas for the next image
                self.click_count = 0
                ...
                if self.current_image_index < len(self.image_list):
                    ...
                    self.application_running = False
                    self.root.destroy()
                    # Call save_coordinates_to_file() after processing the last image
                    self.save_coordinates_to_file()
            else:
                messagebox.showinfo("Error", "Please click the second point after the first.")

        # Directly call save_coordinates_to_file() here to ensure it is called on every click
        self.save_coordinates_to_file()
        ...
        if self.click_count == 2:
            ...
            # Add the following line to call save_coordinates_to_file() after the third click
            self.save_coordinates_to_file()
        else:
            self.show_position(f"Click on image {self.current_image_index + 1}, point {self.click_count + 1}")
            ...
            try:
                with open(filepath, "w") as file:
                    ...
                    for coords_set in self.click_coords:
                        ...
                        coords_line = f"{x1},{y1},{x2},{y2 if y2 < 0 else '-' + str(y2)},1,0,0,0\n"
                        file.write(coords_line)

                        self.get_alpha_beta(self.scale_var.get())
                        x2, y1, x1, y2 = x2, y1 + self.beta, x1 + self.alpha, y2 + self.beta
                        additional_data_line = f"{x2},{y1},{x1},{y2 if y2 < 0 else '-' + str(y2)},1,0,0,0\n"
                        file.write(additional_data_line)

                #messagebox.showinfo("Success", f"Coordinates saved to {points_filename}")
