...
class PixelExtractor:
    ...
    def on_click(self, event):
        ...
        if self.click_count == 0:
            user_x1 = simpledialog.askfloat(f"X Coordinate for point {self.click_count + 1}",
                                            f"Enter X coordinate for point {self.click_count + 1}",
                                            parent=self.image_frame)
            user_y1 = simpledialog.askfloat(f"Y Coordinate for point {self.click_count + 1}",
                                            f"Enter Y coordinate for point {self.click_count + 1}",
                                            parent=self.image_frame)
            pixelx1, pixely1 = x, y
            self.click_coords.append((user_x1, user_y1, pixelx1, pixely1))
        elif self.click_count == 1:
            x1, y1, _, _ = self.click_coords[0]
            pixelx2, pixely2 = x, y
            user_x2, user_y2 = x1 + alpha, y1
            self.click_coords.append((user_x2, user_y2, pixelx2, pixely2))
        elif self.click_count == 2:
            _, _, x2, y2 = self.click_coords[1]
            pixelx3, pixely3 = x, y
            user_x3, user_y3 = x2 + alpha, y2 + beta
            self.click_coords.append((user_x3, user_y3, pixelx3, pixely3))
            ...
            try:
                with open(filepath, "w") as file:
                    ...
                    for coords_set in self.click_coords:
                        user_x, user_y, pixel_x, pixel_y = coords_set
                        coords_line = f"{user_x + alpha},{user_y + beta},{pixel_x},{pixel_y},1,0,0,0\n"
