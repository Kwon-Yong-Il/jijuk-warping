...
class PixelExtractor:
    ...
    def update_magnifier(self, event):
        if self.tk_image:
            ...
            magnified_size = 400  # Set the magnified image size
            crop_left = int(max(x - magnified_size / 20, 0))
            crop_top = int(max(y - magnified_size / 20, 0))
            crop_right = int(min(crop_left + magnified_size / 10, self.original_image.width))
            crop_bottom = int(min(crop_top + magnified_size / 10, self.original_image.height))

            magnified_region = self.original_image.crop((crop_left, crop_top, crop_right, crop_bottom))
            magnified_region = magnified_region.resize((magnified_size, magnified_size), resample=Image.BICUBIC)

            # Update magnifier position to follow the mouse cursor
            if not hasattr(self, 'magnifier_image'):
                self.magnifier_image = self.canvas.create_image(0, 0, anchor=tk.CENTER, image=None)

            self.canvas.coords(self.magnifier_image, x, y)
            self.canvas.itemconfig(self.magnifier_image, image=ImageTk.PhotoImage(magnified_region))
