...
class PixelExtractor:
    ...
    def load_target_dogak_coordinates(self, target_filename):
        ...
        dogak_file_path = os.path.join(self.image_folder_path, 'dogak_list.txt')

        try:
            matched = False  # 파일명이 일치하는지 여부를 저장하는 플래그
            user_x, user_y = 0, 0  # 초기값 설정

            # 파일이 존재하는 경우에만 읽기 모드로 파일 열기
            if os.path.exists(dogak_file_path):
                ...
                for line in lines:
                    ...
                    if len(fields) >= 3:  # 최소한 3개의 필드가 있어야 함 (파일명, x, y)
                        image_filename, x, y = fields[0], float(fields[1]), float(fields[2])

                        # 확장자를 제외하고 공백을 제거한 후 파일명이 일치하는 경우에만 x, y 값을 저장
                        original_filename = os.path.splitext(image_filename)[0].replace(" ", "")
                        target_filename_without_extension = os.path.splitext(os.path.basename(target_filename))[0].replace(" ", "")

                        if original_filename == target_filename_without_extension:
                            user_x, user_y = x, y
                            matched = True  # 파일명이 일치하는 경우 플래그를 True로 설정
                            print(f"Matched: user_x={user_x}, user_y={user_y}")  # 디버깅용 출력
                            break

            # 파일명이 일치하지 않거나 파일이 존재하지 않는 경우 플래그를 확인하고 파일명 뒤에 "_불일치"를 붙여줌
            if not matched:
                file_name_without_extension = os.path.splitext(os.path.basename(target_filename))[0]
                print(f"Not matched: {file_name_without_extension}")  # 디버깅용 출력
                with open(dogak_file_path, "a") as file:
                    file.write(f"{file_name_without_extension}_불일치.points\n")
    ...
    def on_click(self, event):
        ...
        if self.click_count == 0:
            user_x, user_y = self.load_target_dogak_coordinates(self.image_list[self.current_image_index])
            pixelx1, pixely1 = x, y
