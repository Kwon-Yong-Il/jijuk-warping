...
class PixelExtractor:
    ...
    def load_images(self):
        ...
        if not os.path.exists(dogak_file_path):
            messagebox.showinfo("Error", "dogak_list.txt not found in the selected folder.")
            self.root.destroy()
            return

        with open(dogak_file_path, "r") as dogak_file:
            self.image_list = [line.strip().split(',')[0] for line in dogak_file]

        if not self.image_list:
            messagebox.showinfo("Error", "No images found in dogak_list.txt.")
            self.root.destroy()
            return
    ...
    def load_image(self):
        if self.current_image_index < len(self.image_list):
            ...
            image_path = os.path.join(self.image_folder_path, f"{image_filename}.JPG")
                ...
                for line in lines:
                    ...
                    if len(fields) >= 3:  # 최소한 3개의 필드가 있어야 함 (파일명, x, y)
                        ...
                        if os.path.splitext(image_filename)[0] == os.path.splitext(os.path.basename(target_filename))[0]:
                            user_x, user_y = x, y
                            print(f"user_x: {user_x}, user_y: {user_y}")
                            break
