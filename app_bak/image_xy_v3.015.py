...
class PixelExtractor:
    ...
    def load_target_dogak_coordinates(self, target_filename):
        ...
        try:
            ...
            # 쓰기 모드로 파일 열기
            with open(dogak_file_path, "w") as file:
                for line in lines:
                    fields = line.strip().split(',')
                    if len(fields) >= 3:  # 최소한 3개의 필드가 있어야 함 (파일명, x, y)
                        image_filename, user_x, user_y = fields[0], float(fields[1]), float(fields[2])

                        # 파일명이 일치하는 경우에만 x, y 값을 더해주고 저장
                        if os.path.splitext(image_filename)[0] == os.path.splitext(os.path.basename(target_filename))[0]:
                            ...
                            # 예: 수정된 좌표를 사용하여 작업 수행
                            map_x, map_y = 0, 0
                            coords_line = f"{map_x},{map_y},{user_x},{user_y},1,0,0,0\n"
                            file.write(coords_line)

                            matched = True  # 파일명이 일치하는 경우 플래그를 True로 설정
                            print(f"user_x: {user_x}, user_y: {user_y}")
                        else:
                            # 파일명이 일치하지 않는 경우에는 기존 라인 그대로 저장
                            file.write(line)

                # 파일명이 일치하지 않는 경우 플래그를 확인하고 파일명 뒤에 "_불일치"를 붙여줌
                if not matched:
                    file.write(f"{os.path.splitext(os.path.basename(target_filename))[0]}_불일치.points\n")
