    ...
    def update_magnifier(self, event):
        if self.tk_image:
            ...
            if not hasattr(self, 'magnifier_window'):
                ...
                self.magnifier_canvas = tk.Canvas(self.magnifier_window, width=magnified_size, height=magnified_size)
                self.magnifier_canvas.pack()

            tk_image = ImageTk.PhotoImage(magnified_region)
            self.magnifier_canvas.create_image(magnified_size / 2, magnified_size / 2, anchor=tk.CENTER, image=tk_image)
            self.magnifier_canvas.update()

            # Destroy the magnifier window when the mouse leaves the canvas
            self.canvas.bind("<Leave>", lambda e: self.magnifier_window.destroy())
