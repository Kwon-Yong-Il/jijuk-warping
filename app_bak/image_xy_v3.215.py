...
class PixelExtractor:
    ...
    def load_images(self):
        ...
        dogak_list_file_path = filedialog.askopenfilename(
            title="Select a text file",
            filetypes=[("Text files", "*.txt"), ("All files", "*.*")]
        )

        if not dogak_list_file_path:
            messagebox.showinfo("Error", "No text file selected.")
            self.root.destroy()
            return

        # Load images and search for coordinates in the selected text file
        for image_path in self.image_list:
            self.load_image(image_path, dogak_list_file_path)
    ...
    def load_image(self):
        if self.current_image_index < len(self.image_list):
            ...
            # Call the modified load_target_dogak_coordinates method
            user_x, user_y = self.load_target_dogak_coordinates(image_path, dogak_list_file_path)
            ...
            with open(dogak_list_file_path, "r") as file:
                ...
                for line in lines:
                    ...
                    if len(fields) >= 3:
                        ...
                        if os.path.splitext(image_filename)[:15] == os.path.splitext(os.path.basename(target_filename))[:15]:
                            user_x, user_y = x, y
