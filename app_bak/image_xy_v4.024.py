...
class PixelExtractor:
    ...
    def update_magnifier(self, event):
        if self.tk_image:
            ...
            magnifier_x = max(0, min(x - magnified_size / (2 * magnification_factor), self.original_image.width - magnified_size))
            magnifier_y = max(0, min(y - magnified_size / (2 * magnification_factor), self.original_image.height - magnified_size))

            # Calculate the region to crop from the original image
            crop_left = int(magnifier_x * magnification_factor)
            crop_top = int(magnifier_y * magnification_factor)
            crop_right = int((magnifier_x + magnified_size) * magnification_factor)
            crop_bottom = int((magnifier_y + magnified_size) * magnification_factor)
            ...
            self.canvas.create_line(x - center_x, y, x + center_x, y, fill="red", tags="magnifier")

            # Draw vertical line
            self.canvas.create_line(x, y - center_y, x, y + center_y, fill="red", tags="magnifier")
