...
class PixelExtractor:
    ...
    def update_magnifier(self, event):
        if self.tk_image:
            ...
            magnifier_x = max(0, min(x, self.original_image.width - 1 - magnified_size))
            magnifier_y = max(0, min(y, self.original_image.height - 1 - magnified_size))

            # Calculate the region to crop from the original image
            crop_left = int(max(x - magnified_size / (2 * magnification_factor), 0))
            crop_top = int(max(y - magnified_size / (2 * magnification_factor), 0))
            crop_right = int(min(crop_left + magnified_size, self.original_image.width))
            crop_bottom = int(min(crop_top + magnified_size, self.original_image.height))
            ...
            self.canvas.coords(self.magnifier_image, magnifier_x + magnified_size / 2, magnifier_y + magnified_size / 2)
            ...
            center_x = magnified_size / 2
            center_y = magnified_size / 2

            # Draw horizontal line
            self.canvas.create_line(magnifier_x, y, magnifier_x + magnified_size, y, fill="red", tags="magnifier")

            # Draw vertical line
            self.canvas.create_line(x, magnifier_y, x, magnifier_y + magnified_size, fill="red", tags="magnifier")
