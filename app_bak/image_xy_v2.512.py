...
class PixelExtractor:
    ...
    def on_click(self, event):
        ...
        if self.current_image_index < len(self.image_list) and self.click_count == 0:
            # If there are more images and it's the first click, load the next image
            self.load_image()


    def save_to_excel(self, image_filename, positions, alpha, beta):
        clicked_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
