...
class PixelExtractor:
    ...
    def load_target_dogak_coordinates(self, target_filename):
        dogak_file_path = os.path.join(os.path.dirname(target_filename), 'dogak_list.txt')
            ...
            if os.path.exists(dogak_file_path):
                with open(dogak_file_path, "r") as file:
                    ...
                    for line in lines:
                        ...
                        # Parse the line and check if the filename matches the target filename
                        fields = line.strip().split(',')
                        if len(fields) >= 3:
                            image_filename, x, y = fields[0], float(fields[1]), float(fields[2])

                            # Check if the image filename matches the target filename
                            if len(os.path.splitext(image_filename)) >= 1 and len(os.path.splitext(os.path.basename(target_filename))) >= 1:
                                if os.path.splitext(image_filename)[0][:15] == os.path.splitext(os.path.basename(target_filename))[0][:15]:
                                    user_x, user_y = x, y
