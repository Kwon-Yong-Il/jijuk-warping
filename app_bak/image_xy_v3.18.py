...
class PixelExtractor:
    ...
    def process_next_image(self):
        ...
        # Save the last line for the last image
        self.save_to_excel(image_filename, [(self.click_coords[1][0] + self.alpha, self.click_coords[1][1], self.click_coords[1][2], self.click_coords[1][3])])
    ...
    def save_coordinates_to_file(self):
        for i, coords_set in enumerate(self.click_coords, start=1):
            user_x, user_y, x, y = coords_set
            alpha, beta = self.get_alpha_beta(self.scale_var.get())
            user_x_last = user_x + alpha
            user_y_last = user_y + beta
            image_filename = os.path.splitext(os.path.basename(self.image_list[i - 1]))[0]
            points_filename = image_filename + ".points"
            image_directory = os.path.dirname(self.image_list[i - 1])
            ...
            try:
                with open(filepath, "w") as file:
                    ...
                    coords_line = f"{user_x_last},{user_y_last},{x},{y},1,0,0,0\n"
            ...
            try:
                with open(filepath, "w") as file:
                    ...
                    for coords_set in self.click_coords:
                        user_x1, user_y1, user_x2, user_y2 = coords_set
                        coords_line = f"{user_x1},{user_y1},{user_x2},{user_y2},1,0,0,0\n"
                        file.write(coords_line)
