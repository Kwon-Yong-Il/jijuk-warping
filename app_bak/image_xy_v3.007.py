            ...
            try:
                with open(dogak_file_path, "r") as file:
                    ...
                    if len(lines) >= 3:  # 예: 파일에 최소한 3개의 좌표가 있어야 함
                        ...
                        _, _, pixel_x1, pixel_y1, *_ = map(float, lines[1].strip().split(",")[0:4])  # 수정된 부분
                        _, _, pixel_x2, pixel_y2, *_ = map(float, lines[2].strip().split(",")[0:4])  # 수정된 부분

                        # 예: 추출된 좌표를 사용하여 작업 수행
                        # 여기에서는 간단히 출력만 하도록 하였습니다. 실제로 필요한 로직으로 변경해주세요.
                        print(f"pixel_x1: {pixel_x1}, pixel_y1: {pixel_y1}")
                        print(f"pixel_x2: {pixel_x2}, pixel_y2: {pixel_y2}")
