...
class PixelExtractor:
    ...
    def on_click(self, event):
        ...
        if self.click_count == 0:
            # 사용자에게 x, y 값 입력
            user_x = simpledialog.askfloat("X Coordinate", f"Enter X coordinate for point {self.click_count + 1}", parent=self.image_frame)
            user_y = simpledialog.askfloat("Y Coordinate", f"Enter Y coordinate for point {self.click_count + 1}", parent=self.image_frame)
            pixelx1, pixely1 = x, y
            ...
            pixelx3, pixely3 = x, y
            user_x = pixelx2 + alpha  # Fix: Calculate user_x based on pixelx2
            user_y = pixely2 + beta   # Fix: Calculate user_y based on pixely2
        ...
        self.save_coordinates_to_file(alpha, beta)  # Pass alpha and beta as arguments
            ...
            try:
                with open(filepath, "w") as file:
                    ...
                    for coords_set in self.click_coords:
                        user_x1, user_y1, pixelx1, pixely1 = coords_set
                        mapx1 = user_x1
                        mapy1 = user_y1
                        coords_line = f"{mapx1 + alpha},{mapy1 + beta},{pixelx1},{pixely1},1,0,0,0\n"
                        coords_line = f"{mapx1 + alpha},mapy1,{pixelx1},{pixely1},1,0,0,0\n"
