...
class PixelExtractor:
    def __init__(self, root):
        self.root = root
        self.application_running = True  # New attribute
        self.root.protocol("WM_DELETE_WINDOW", self.on_close)  # Register the callback for closing the window
    ...
    def load_image(self):
        if self.current_image_index < len(self.image_list):
            ...
            # Create a copy of the image to avoid modifying the original file
            self.original_image = Image.open(image_path).copy()

            # Display the copied image in its original size
            self.tk_image = ImageTk.PhotoImage(self.original_image)
            self.image_item = self.canvas.create_image(0, 0, anchor=tk.NW, image=self.tk_image)

            image_filename = os.path.splitext(os.path.basename(image_path))[0]  # remove extension
            self.image_name_label.config(text=f"{image_filename}")
    ...
    def on_click(self, event):
        ...
        if self.current_image_index >= len(self.image_list):
            self.application_running = False  # Set the flag to indicate the application is closing
            self.workbook.save("image.xlsx")
        ...
        if self.click_count == 0:
            ...
            if user_x is not None and user_y is not None:
                ...
                self.save_to_excel(image_filename, (user_x, user_y, x, y))
                self.save_to_excel(image_filename, (self.click_coords[1][0], self.click_coords[1][1],
                                                    self.click_coords[0][2], self.click_coords[0][3]))
        ...
        # Check if the application is still running before accessing widgets
    ...
    def show_position(self, text):
        # Check if the Tkinter application is still running and if the label widget exists
